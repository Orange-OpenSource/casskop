...
# CMDs
UNIT_TEST_CMD := KUBERNETES_CONFIG=`pwd`/config/test-kube-config.yaml POD_NAME=test go test -mod vendor --cover --coverprofile=coverage.out `go list ./... | grep -v e2e` > test-report.out 
