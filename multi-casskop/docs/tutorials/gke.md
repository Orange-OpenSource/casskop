...
To test external dns works correctly : 

```sh
cat <<EOF | sed "s/\[NAMESPACE\]/$NAMESPACE/" | kubectl apply -f -
---
apiVersion: v1
kind: Service
metadata:
  name: nginx
  annotations:
    external-dns.alpha.kubernetes.io/hostname: ngnix.external-dns-test.gcp.trycatchlearn.fr.
spec:
  type: LoadBalancer
  ports:
  - port: 80
...
$ gcloud dns record-sets list \
    --zone "external-dns-test-gcp-trycatchlearn-fr" \
    --name "nginx.external-dns-test.gcp.trycatchlearn.fr."
$ curl nginx.external-dns-test.gcp.trycatchlearn.fr
```